...
def parse_item_list_txt(txtpath):
    ...
    if sample.split("/")[0] in str(basedir):
        raise NotImplementedError(
            "Not implemented: file path in txt and basedir have comman str."
        )
    else:
...
class ImagesDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, opt, ds_type=TRAIN_DATA, transform=None, batchsize=None):
        ...
        print(
            f"{ds_type} - Input Directory path: [yellow]{input_globs}[/yellow]"
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.have_gt:
            ...
            if res_item[GT_FPATH].endswith(".hdr"):
                ...
                gt_img = cv2.imread(
                    self.gt_list[idx], flags=cv2.IMREAD_ANYDEPTH
                )[:, :, [2, 1, 0]]
...
class DataModule(LightningDataModule):
    def __init__(
        self, opt, apply_test_transform=False, apply_valid_transform=False
    ):
        super().__init__()
        self.opt = opt
    ...
    def setup(self, stage):
        ...
        if stage == "fit":
            ...
            self.training_dataset = ImagesDataset(
                opt,
                ds_type=TRAIN_DATA,
            ...
            if opt[VALID_DATA] and opt[VALID_DATA][INPUT]:
                self.valid_dataset = ImagesDataset(
                    opt,
                    ds_type=VALID_DATA,
                    transform=self.valid_transform,
                    batchsize=opt.valid_batchsize,
                )

            # no valid data, splt from training data:
            elif opt[VALID_RATIO]:
                print(
                    f"Split valid dataset from training data. Ratio: {opt[VALID_RATIO]}"
                )
                valid_size = int(opt[VALID_RATIO] * len(self.training_dataset))
                train_size = len(self.training_dataset) - valid_size
                torch.manual_seed(233)
                self.training_dataset, self.valid_dataset = (
                    torch.utils.data.random_split(
                        self.training_dataset, [train_size, valid_size]
                    )
                )
                print(
