...
class Downsample:
    ...
    def __call__(self, img):
        ...
        if isinstance(self.downsample_factor, Iterable):
            ...
            raise RuntimeError(
                f"ERR: Wrong config aug.downsample: {self.downsample_factor}"
            )
...
def parseAugmentation(opt):
    ...
    augmentaionFactory = {
        DOWNSAMPLE: (
            Downsample(aug_config[DOWNSAMPLE])
            if get_value(aug_config, DOWNSAMPLE)
            else None
        ),
        CROP: (
            transforms.RandomCrop(aug_config[CROP])
            if get_value(aug_config, CROP)
            else None
        ),
