...
class HistUNet(nn.Module):
    def __init__(
        ...
        if guide_feature_from_hist:
            kargs = {
                "guide_input_channel": (
                    n_bins if use_gray_hist else n_bins * in_channels
                )
            }
        else:
            kargs = {}
        ...
        self.down1 = Down(
            C_NUMS[0] + encoder_extra_c_num,
            C_NUMS[1],
            use_hist=encoder_use_hist,
        )
        self.down2 = Down(
            C_NUMS[1] + encoder_extra_c_num,
            C_NUMS[2],
            use_hist=encoder_use_hist,
        )
        self.down3 = Down(
            C_NUMS[2] + encoder_extra_c_num,
            C_NUMS[3],
            use_hist=encoder_use_hist,
        )
        self.down4 = Down(
        ...
        self.up1 = Up(
            C_NUMS[4] + extra_c_num, C_NUMS[3] // factor, bilinear, **kargs
        )
        self.up2 = Up(
            C_NUMS[3] + extra_c_num, C_NUMS[2] // factor, bilinear, **kargs
        )
        self.up3 = Up(
            C_NUMS[2] + extra_c_num, C_NUMS[1] // factor, bilinear, **kargs
        )
